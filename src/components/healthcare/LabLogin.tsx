...
import { FlaskConical, Key, LogIn, AlertCircle, X, Eye, EyeOff } from "lucide-react";
import { Button } from "@/components/ui/button";

export default function LabLogin() {
  const { labLogin, closeLabPanel } = useLabStore();
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (!labId.trim() || !password.trim()) {
      setError("Please enter both Lab ID and Password");
      return;
    }
    setIsLoading(true);
    setTimeout(() => {
      const result = labLogin(labId.trim(), password.trim());
      if (!result) {
    ...
    <div className="fixed inset-0 z-[100] bg-white flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-1">Lab Portal</h1>
        ...
        <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-lg">
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
              <label className="text-sm font-semibold text-gray-700 mb-1.5 block">Lab ID</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full h-12 rounded-xl bg-gray-50 border border-gray-200 pl-10 pr-4 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#1e3a5f]/30 focus:border-[#1e3a5f] transition-all"
                />
              </div>
            </div>
            <div>
          ...
          <div className="mt-5 pt-4 border-t border-gray-100 text-center">
            <p className="text-gray-400 text-xs">Demo: LAB-1001 / LabPass2k24</p>
          </div>
        </div>
      </div>
    </div>
